...
function showCoolNotification(message) {
    Swal.fire({
        ...
        text: message,
        showConfirmButton: false,
        timer: 1200, // Mais rápido (de 2000 para 1200)
        toast: true,
        width: '250px', // Menor e mais discreto
        didOpen: (toast) => {
...
function abrirAssistente() {
    ...
    const btn = document.getElementById('btnAbrirAssistente');
    if (assistente) assistente.style.setProperty('display', 'block', 'important');
    if (btn) btn.style.setProperty('display', 'none', 'important');
    
    assistente.style.animation = 'none';
    setTimeout(() => {
        assistente.style.animation = 'surgir 0.3s ease-out';
    }, 10);
}
...
function fecharAssistente() {
    const assistente = document.getElementById('assistente');
    const btn = document.getElementById('btnAbrirAssistente');
    if (assistente) assistente.style.setProperty('display', 'none', 'important');
